...
def parseargs():
    ...
    parser.add_argument(
        "-p",
        "--path",
        help="The path to the directory that contains malware and goodware image folders",
        type=str,
        required=True,
    )
    parser.add_argument(
        "-d",
        "--dir",
        help="The name of the directory where to save the model",
        type=str,
        required=True,
    )
    parser.add_argument(
        "-f",
        "--file",
        help="The name of the file where to save the results of the evaluation",
        type=str,
        required=True,
    )
...
def main(
    ...
    for i in range(1, 11):
        ...
        with open(os.path.join(PATH_FILES, "train" + str(i) + ".txt")) as f:
            ...
            train_imgs = [
                os.path.join(path_images, image_hash) for image_hash in train_hashes
            ]
        f.close()
        ...
        with open(os.path.join(PATH_FILES, "valid" + str(i) + ".txt")) as f:
            ...
            valid_imgs = [
                os.path.join(path_images, image_hash) for image_hash in valid_hashes
            ]
        f.close()
        ...
        with open(os.path.join(PATH_FILES, "test" + str(i) + ".txt")) as f:
            ...
            test_imgs = [
                os.path.join(path_images, image_hash) for image_hash in test_hashes
            ]
        f.close()
        ...
        cp_callback = tf.keras.callbacks.ModelCheckpoint(
            filepath=os.path.join(dir_name, "cp" + str(i)),
